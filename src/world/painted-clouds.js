...
export function createPaintedClouds(planet){
 ...
 const material=new THREE.ShaderMaterial({transparent:true,depthWrite:false,uniforms:{map:{value:map},time:{value:0},haze:{value:new THREE.Color(planet.horizon)}},vertexShader:`uniform float time;varying vec2 cloudUV;varying float cloudDistance;void main(){cloudUV=uv;vec4 center=modelMatrix*instanceMatrix*vec4(0.,0.,0.,1.);center.x+=sin(time*.009+center.z*.003)*50.;vec4 mv=viewMatrix*center;mv.xy+=position.xy*vec2(length(instanceMatrix[0].xyz),length(instanceMatrix[1].xyz));cloudDistance=-mv.z;gl_Position=projectionMatrix*mv;}`,fragmentShader:`uniform sampler2D map;uniform vec3 haze;varying vec2 cloudUV;varying float cloudDistance;void main(){vec4 c=texture2D(map,cloudUV);if(c.a<.035)discard;vec3 paint=c.rgb;paint=mix(paint,haze,smoothstep(500.,6500.,cloudDistance)*.6);gl_FragColor=vec4(paint,c.a*.82);
