...
import {createPaintedHorizon} from './painted-horizon.js';
import {illustrateMaterial} from './art-direction.js';
import {createPaintedClouds} from './painted-clouds.js';
import {getRock,createExplorer} from './asset-library.js';
...
float breakers=(1.-smoothstep(0.,12.,waterDepth))*smoothstep(.87,.96,.5+.5*sin(waterDepth*1.65-time*1.2+ripple));c=mix(c,vec3(.91,.89,.72),breakers*.55);
float sunPath=pow(max(dot(reflect(-view,n),normalize(vec3(-.65,.5,-.55))),0.),48.);c+=vec3(.95,.6,.23)*sunPath*(.2+ripple*.5);
if(lava>.5){vec2 flow=vP.xz*.014+vec2(time*.035,-time*.021);float cells=noise(vec3(flow,1.))+noise(vec3(flow*2.1,time*.03))*.2;float seams=1.-smoothstep(.025,.065,abs(cells-.6));vec3 crust=vec3(.045,.018,.038)*(.75+cells*.5);c=mix(crust,vec3(2.4,.28,.016),seams);c+=vec3(.3,.035,.005)*(.5+.5*sin(cells*9.+time*.7));}
...
#include <colorspace_fragment>
  ...
  canopyMat.onBeforeCompile=shader=>{shader.vertexShader=shader.vertexShader.replace('#include <common>','#include <common>\nvarying vec3 fungiPosition;').replace('#include <begin_vertex>','#include <begin_vertex>\nfungiPosition=position;');shader.fragmentShader=shader.fragmentShader.replace('#include <common>','#include <common>\nvarying vec3 fungiPosition;\n'+noiseGLSL).replace('#include <color_fragment>','#include <color_fragment>\nfloat mottling=noise(fungiPosition*5.);float speckles=smoothstep(.60,.69,mottling);diffuseColor.rgb=mix(diffuseColor.rgb*(.85+mottling*.4),vec3(.94,.64,.39),speckles*.18);');};
  ...
  if(planet.tree==='mushroom'){
    const curve=new THREE.CatmullRomCurve3([new THREE.Vector3(0,0,0),new THREE.Vector3(.06,.32,.02),new THREE.Vector3(-.03,.7,.04),new THREE.Vector3(0,1,0)]);
    instances(new THREE.TubeGeometry(curve,10,.055,8,false),trunkMat,treeData,(d,t)=>{d.scale.setScalar(t.s);});
    const profile=[[0,.52],[.25,.51],[.56,.46],[.79,.36],[.94,.23],[1,.14],[.98,.08],[.85,.035],[.5,0],[0,0]].map(p=>new THREE.Vector2(...p));
    instances(new THREE.LatheGeometry(profile,16),canopyMat,treeData,(d,t)=>{d.position.y+=t.s*.96;d.scale.set(t.s*.76,t.s*.57,t.s*.76);});
    const underProfile=[[0,-.08],[.35,-.05],[.7,0],[.96,.07]].map(p=>new THREE.Vector2(...p));
    instances(new THREE.LatheGeometry(underProfile,12),undersideMat,treeData,(d,t)=>{d.position.y+=t.s*.96;d.scale.set(t.s*.76,t.s*.57,t.s*.76);});
    ...
    instances(new THREE.LatheGeometry(profile,18),canopyMat,pods,(d,t)=>{d.position.x+=t.s*.17;d.position.y+=t.s*.5;d.scale.set(t.s*.32,t.s*.30,t.s*.32);});
    ...
    instances(new THREE.CylinderGeometry(.13,.20,1,7),canopyMat,treeData,(d,t)=>{d.position.y+=t.s*.5;d.scale.setScalar(t.s);});
    instances(new THREE.SphereGeometry(.21,7,6),canopyMat,treeData,(d,t)=>{d.position.y+=t.s;d.scale.set(t.s,t.s*.6,t.s);});
    instances(new THREE.CylinderGeometry(.08,.11,.6,6),canopyMat,treeData,(d,t)=>{d.position.x+=t.s*.23;d.position.y+=t.s*.6;d.rotation.z=-.8;d.scale.setScalar(t.s);});
  }
  ...
  for(let variant=0;variant<3;variant++){const asset=getRock(variant);const paintedRock=new THREE.MeshStandardMaterial({color:planet.rock,roughness:1,flatShading:true});instances(asset.geometry,paintedRock,rockData.filter((_,i)=>i%3===variant),(d,t)=>{d.position.y+=t.s*.3;d.scale.set(t.s,t.s*.6,t.s*.85);});}
...
#include <colorspace_fragment>
  ...
  const paintedClouds=createPaintedClouds(planet);group.add(paintedClouds.mesh);
  const moonMat=new THREE.ShaderMaterial({vertexShader:`varying vec3 p;varying vec3 n;void main(){p=position;n=normal;gl_Position=projectionMatrix*modelViewMatrix*vec4(position,1.);}`,fragmentShader:`varying vec3 p;varying vec3 n;${noiseGLSL}void main(){float terrain=fbm(p*.009);float l=max(dot(n,normalize(vec3(-1,1,1))),0.);vec3 c=mix(vec3(.31,.42,.49),vec3(.74,.75,.70),terrain);gl_FragColor=vec4(c*(.35+l*.7),1.);
#include <tonemapping_fragment>
#include <colorspace_fragment>
  ...
  const sphereGeo=new THREE.SphereGeometry(1,10,7),limbGeo=new THREE.CylinderGeometry(.13,.1,1.7,6);
